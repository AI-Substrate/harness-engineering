...
## Local checks & the pre-push gate

- This repo installs a **`pre-push` git hook** via `core.hooksPath=.githooks` (enabled once per clone with `just install-hooks`). On every `git push` it runs `npm run build` then the full **`harness checks`** suite — the same composite gate CI runs (tests+coverage, biome, typecheck, the docs/flows/telemetry drift guards, and arch/skills/markdown/windows-check).
- **The gate is heavyweight and can run for many minutes** (the smoke suite dominates). A `git push` will therefore appear to *hang* with no output while the gate runs — it is not a network stall; it is the hook. Budget several minutes, or run `harness checks` yourself first to see progress.
- It blocks the push only on a **hard-gate failure** (`checks` exits 1); warn-launch / `degraded` findings (exit 0) pass through.
- Skip it in a pinch with `git push --no-verify` — justified when you have *already* run the identical gate (build + `harness checks`) in this session. The hook is fast local feedback, **not** the authoritative gate: **CI + branch protection are**.
