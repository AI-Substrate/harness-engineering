/**
 * TEMPORARY COPY of dd `core/rel` — no public `@ai-substrate/dd` home at the pin
 * (`@ai-substrate/dd/core/rel` → ERR_PACKAGE_PATH_NOT_EXPORTED).
 *
 * The public surface cannot answer this question: `DdLinkCell.rel` carries the
 * DECLARED relation (resolved via `relOf`), not the effective-builtin mapping, so
 * there is no published route from "what the schema wrote" to "what dd means by
 * it". This is a MECHANISM, so unlike ./constants.ts it is genuinely temporary —
 * it goes away when dd's mechanism-vocabulary seam lands (dd `6aaef35`).
 *
 * The types it reads (`DdShape`, `ResolvedDdSchema`) DO have a public home and are
 * therefore imported from `@ai-substrate/dd/core/model`, never copied — that is
 * the dw-000f line.
 *
 * See ./README.md.
 */

import type { DdShape, ResolvedDdSchema } from '@ai-substrate/dd/core/model';
