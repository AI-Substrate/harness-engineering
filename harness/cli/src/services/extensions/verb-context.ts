 ...
 * the right constructor, guaranteeing a `next_action` on every non-`ok` status
 * (P5) even if the author returned a raw object without one.
 */
  ...
  switch (result.status) {
    ...
    case 'degraded':
      return formatDegraded(
        name,
        result.data,
        result.next_action ?? 'Review the degraded result above.',
        clock,
    ...
    case 'unconfigured':
      return formatUnconfigured(
        name,
        result.next_action ?? 'No behaviour is mapped for this verb yet.',
        clock,
    ...
    case 'error':
      return formatError(
        name,
        result.error?.code ?? ErrorCodes.UNKNOWN,
        result.error?.message ?? 'The verb reported an error.',
        clock,
        {
