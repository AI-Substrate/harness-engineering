...
/** Core command names an extension may NOT shadow (the bare orientation is argless). */
export const RESERVED_NAMES: ReadonlySet<string> = new Set(['help', 'doctor']);
 ...
 * claimed by a core command or an earlier extension is a `conflict` (`E142`); the
 * shadowed duplicate is recorded, not registered. Verb names are an open
 * `string` key — no closed union (AC-7).
 */
    ...
    records.push(
      shadowed === undefined
        ...
        : {
            ...
            verbs: accepted,
            shadows: shadowed,
            error: `${ErrorCodes.EXTENSION_VERB_CONFLICT}: verb '${shadowed}' is already provided (core command or earlier extension); duplicate ignored.`,
          },
    );
  }
