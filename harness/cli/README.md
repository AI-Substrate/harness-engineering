...
| `harness builder` | Architecture-enabled team delivery: new, adopt, guide, ready, contracts, settings, dispatch, ack, advance, compose, review, close and tidy. Read `harness instructions builder` or `harness docs harness-builder` before operating it. | core |
| `harness <verb> […]` | Any verb a discovered extension contributes, with its own `--help`, options, args, Envelope, and exit code. | 🧩 extension |

`help`, `doctor`, `new`, `builder`, `docs`, `skills`, `update`, and `self-install` are **reserved** core commands — no extension can shadow them (doctor is the diagnostic that *checks* the extension system). Safe mode: `--no-extensions` or `HARNESS_NO_EXTENSIONS=1` skips discovery entirely (core commands only).
