...
**before considering work done**, and the gate a team gates commits/push on. It
**composes** the repo's existing deterministic checks into a single honest envelope
(read-only — it never auto-fixes, unlike `just fft`):

| Gate | Runs | Posture |
|---|---|---|
| `tests` | `npx vitest run` (harness/cli) | **hard** — `error` on failure |
...
`data` carries `{ durationMs, summary, gates[] }`, where each gate is
`{ name, status, exit, note }`.
...
|---|---|---|
| every gate `ok` | `ok` | 0 |
| any hard gate (`tests`/`skills-check`) `error` | `error` (`E_CHECKS_FAILED`) | 1 |
