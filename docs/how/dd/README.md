...
same operation — you never think about it. `node_modules/.bin/dd build` exists for the
...
```bash
node_modules/.bin/dd get docs/how/dd/exemplar/plan.dd.json#acceptance_criteria/ac-0201/state
# → "checked"
```
...
demonstrates it. "What proves this claim?" is one `node_modules/.bin/dd graph` walk.
...
contradictions surface as findings. `node_modules/.bin/dd doctor` sweeps the whole
...
```bash
node_modules/.bin/dd build note.dd.json --check
...
`node_modules/.bin/dd get/set/add/rm` validate before the write, rebuild the sibling
in the same op, and mint collision-free ids (`add --mint`). A refusal writes
nothing.

```bash
node_modules/.bin/dd set "backpressure.dd.json#rows/bp-7106/probe" "npx vitest run ..."
...
```bash
node_modules/.bin/dd validate example/note.dd.json --depth 0
node_modules/.bin/dd build example/note.dd.json
node_modules/.bin/dd build example/note.dd.json --check
```
...
```bash
node_modules/.bin/dd docs list
node_modules/.bin/dd docs get dd-overview
node_modules/.bin/dd docs get how-to-add-a-schema
```
