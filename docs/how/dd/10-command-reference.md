...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs validate <path>` | validate a document and three outbound hops |
| `node_modules/.bin/ddocs validate <path> --depth 0` | validate only the named document |
| `node_modules/.bin/ddocs validate <path> --depth <n>` | validate the named outbound radius |
| `node_modules/.bin/ddocs build <path>` | write the canonical `.dd.md` sibling |
| `node_modules/.bin/ddocs build <path> --check` | compare the canonical render without writing |
| `node_modules/.bin/ddocs doctor` | sweep the repository at unlimited radius |
| `node_modules/.bin/ddocs doctor --path <dir>` | scope the sweep's starting set |
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs schema list` | list winning repository, harness, and home schemas |
| `node_modules/.bin/ddocs schema show <package>/<schema>` | show one resolved package in detail |
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs address generate <interior>` | generate a bare same-document address |
| `node_modules/.bin/ddocs address generate <interior> --path <file>` | generate a qualified address |
| `node_modules/.bin/ddocs address validate <address>` | check syntax and normalization |
| `node_modules/.bin/ddocs address validate <address> --resolve` | resolve and classify every segment |
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs link resolve <address>` | return the addressed value and target metadata |
| `node_modules/.bin/ddocs link verify-basis <address> --sha <sha>` | compare a recorded SHA with the target |
| `node_modules/.bin/ddocs link verify-basis <address> --sha <sha> --update <doc>` | move an existing ledger entry and regenerate its sibling |
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs links <target>` | report inbound and outbound edges for one document |
| `node_modules/.bin/ddocs graph` | return the repository graph as Mermaid and structured data |
| `node_modules/.bin/ddocs graph --path <dir>` | seed the graph from a subtree |
| `node_modules/.bin/ddocs graph map <address>` | walk one row's neighbourhood in both directions |
| `node_modules/.bin/ddocs graph map <address> --direction in` | only what points AT this row |
| `node_modules/.bin/ddocs graph map <address> --rel <rel>` | follow only edges carrying that relation (repeatable) |
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs get <address>` | return the value at any address |
| `node_modules/.bin/ddocs set <address> <value>` | replace the value at an address |
| `node_modules/.bin/ddocs set <address> <json> --value-json` | replace it with parsed JSON (arrays, objects, numbers) |
| `node_modules/.bin/ddocs add <address> <json>` | append an item to a list |
| `node_modules/.bin/ddocs add <address> <json> --mint <prefix>` | append it with a fresh collision-free id |
| `node_modules/.bin/ddocs rm <address>` | remove the item at an address |
...
```bash
node_modules/.bin/ddocs validate docs/how/dd/exemplar/review/round-1.dd.json
...
| Command | Effect |
| --- | --- |
| `node_modules/.bin/ddocs docs list` | list documentation compiled into the CLI |
| `node_modules/.bin/ddocs docs get dd-overview` | print the built-in overview |
| `node_modules/.bin/ddocs docs get how-to-add-a-schema` | print the schema and adapter guide |
...
```bash
node_modules/.bin/ddocs validate path/to/document.dd.json --json \
...
```bash
node_modules/.bin/ddocs graph --path path/to/corpus --json \
  | jq '.data.counts'
```
