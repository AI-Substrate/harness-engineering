...
Use `--already-integrated` only with `--import`, never with `--verify`. By default, every supplied unit must already be present at the current committed PM HEAD. Normal import replays commits; this mode compares immutable Git trees and writes the composition receipt without fetching, cherry-picking or changing HEAD, index or working source. Original worker `UnitDelivery.commit_sha` values remain unchanged even when the PM integrated equivalent trees through different commits.

If later PM edits changed those paths, add `--integration-sha <ref>` alongside `--import --already-integrated`. Builder resolves it to a full commit and requires it to descend from the sealed `source_sha` and be an ancestor of current HEAD. Compare every unit at that historical point and record it as `integration_sha`; keep the current branch, HEAD, index and working source untouched. Subsequent `compose --verify <current-sha>` treats the integration-point-to-artifact delta as PM edits with ownership warnings. No automatic history search or temporary checkout is needed; detached HEAD remains unsupported.
