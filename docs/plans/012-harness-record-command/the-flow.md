...
**Rail**: `[the-flow] ◆─◆─◆─◇─◇`   ·   **now**: Build (harness record) · **next**: Merge
...
flowchart TD
    ...
    D[Design · 3 workshops]:::done --> S[Spec · CS-3 · Simple]:::done --> PL[Plan · READY]:::done --> B[Build · harness record<br/>22 tasks]:::known --> M[Merge]:::assumed
...
_Spec + Plan done and validated (plan fixed 1 CRITICAL ordering bug). Next: `/plan-6` implements the single-phase, 22-task plan (Group B — loader widening — is the risk cluster). No harness-loop nodes (repo has no governance doc)._
