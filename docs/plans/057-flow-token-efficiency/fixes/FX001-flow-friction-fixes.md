...
- [ ] All six Done-When columns hold with evidence in the execution log
- [ ] `just test` + `harness checks` green (degraded = pre-existing warn-tier only)
- [ ] No default-output byte changes anywhere except the two documented additive surfaces (doctor `--quiet`, mutation md side-effect)
