...
2. **Agent identity is CLI-resolved** (D-11): the buffer owner resolves as `--agent <slug>` flag → `HARNESS_AGENT` env var → otherwise the command returns `unconfigured` (exit 2) with `next_action` naming both options. Two agents with different slugs write to different buffers (per-agent isolation preserved); identity is never silently defaulted.
...
7. **Structured read-back + clear** (D-12 defaults, vetoable): `harness observe --list --json` returns the calling agent's pending observations verbatim (including `suggested_encoding`, `first_seen_at`-equivalent timestamps, kind/target/severity) as structured envelope data; `harness observe --clear` truncates the buffer (keeps the file) and returns `ok`. The drain consumes these instead of hand-parsing markdown.
8. **Compaction resilience, end to end**: knowing only `npx harness instructions`, a zero-context agent learns the capture verb (including identity via `--agent`/`HARNESS_AGENT`) and the drain flow (core briefing updated); an observation captured before a simulated context wipe is returned by `--list` after it.
9. **Drain integration**: the merged skill's `--drain` reads pending observations via `--list --json`, presents the `[s/t/p/e/d/a]` menu, materializes saved entries via `harness record retro` (using the returned `data.path` — paths never hard-coded in prose), and clears the buffer via `--clear` — proven by one real dogfood session in this repo.
...
14. **Preserved-behaviors inventory** (the "without watering down" proof — each item present in the merged SKILL.md, verified by review checklist):
    ...
    (c) cross-session leftover check at auto-firing skill start;
...
**Assumptions**: the universal retro schema v1.0 stays frozen; `harness record retro` remains the sole materialization path for committed retros (records land under `.harness/records/retro/<date>/<NNN>-<slug>.md` — the dated-subdir layout; the old skill prose's flat-path example is stale and dies with the rewrite); per-agent buffer isolation via D-11 identity resolution remains the concurrency model; the buffer is not file-locked (race accepted, above).
...
- **D-11: Agent identity resolution** *(added at validation — was the spec's one CRITICAL gap)* → **default: `--agent <slug>` flag, falling back to `HARNESS_AGENT` env var, else `unconfigured` exit 2 naming both options.** Never silently defaulted — a wrong-bucket write is worse than a rejected one. The core briefing teaches `export HARNESS_AGENT=<slug>` once per session. **User may veto.**
- **D-12: Read-back / clear flag shapes** *(added at validation — AC-7/AC-9 needed a locked contract)* → **default: `harness observe --list [--json]` and `harness observe --clear`.** Locked now so the merged SKILL.md prose can name exact commands; /plan-3 may add sugar but not rename. **User may veto.**
