...
- **AC-04** — Given a backpressure chore that is `skipped` **and** carries a `decision` receipt **whose `basis_sha256` equals the target plan's current SHA-256**, the survey dimension reads **satisfied** (a decline is a legitimate ready).
...
| `harness/cli/src/commands/plan.ts` | `cli/commands` | contract | Register `ready` subcommand; envelope + exit mapping |
...
| [x] | T006 | Register `harness plan ready <target>` with envelope + exit mapping: ready→`ok`(0), not-ready→`degraded`(0), cant-tell→`unconfigured`(2) | `cli/commands` | `src/commands/plan.ts` | `harness plan ready --help` lists it; exits match per constitution Principle 6 | |
| [x] | T007 | Add `--strict`: not-ready exits non-zero; default behaviour unchanged | `cli/commands` | `src/commands/plan.ts` | AC-07 holds both ways | Per finding 02 |
