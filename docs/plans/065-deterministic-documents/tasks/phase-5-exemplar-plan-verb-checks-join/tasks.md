...
flowchart TD
    subgraph P5["Phase 5 — exemplar, plan verb & checks join"]
        ...
        T002 --> T006["T006 e2e validation"]:::completed
        T003 --> T006
...
|------|------|------|-----------|------------|------------|
| 2026-08-04 | T001 | Noteworthy | A basis ledger with a cycle has no fixed point: pinning A's sha changes A's bytes, invalidating B's record of A, and so on | Dropped the ledgers that recorded a *citation* rather than a transclusion — a ledger entry means "I transclude this", not "I mention this". The graph is acyclic and pins in one pass | execution.log.md § T001 |
| 2026-08-04 | T001 | Deferred | The "clever use of primitives" bar is a human judgement and stays unresolved | Recorded IN the corpus as `bp-0902`, an unchecked human-judgement row, rather than assumed | exemplar/backpressure.dd.json |
| 2026-08-04 | T001 | Win | Authoring the exemplar found a linkage gap no fixture corpus could have: a dynamic-key map was unsteppable, so workshop-002 Ruling 3 could not resolve at all | OD-8 companion grant on `links/resolver.ts`; dd doctor on the exemplar went 9 ERRORs -> 0 | WIN-001 |
| 2026-08-04 | OD-8 | Noteworthy | `parseShape` builds `DdShape` from an allow-list, so an unread key is silently DROPPED — the ratified grant would have been a no-op | Checked the parser implementation instead of trusting the contract; `schema/declarations.ts` joined the grant | execution.log.md § OD-8 |
| 2026-08-04 | T003 | Difficulty | `dd doctor` swept `.harness/temp`, the declared gitignored scratch dir — the first whole-repo sweep was degraded entirely by my own proof artifacts | `SCAN_SKIP_PATHS`, matched by position not name, yielding to an explicit root. Third member of the "not-repo-content must never shape a gate" class | DL-002 |
| 2026-08-04 | T003 | Difficulty | Ambient `GIT_CONFIG_*` broke the git integration fixture in four consecutive phases; it looks like a code defect and is a host defect | The fixture's own `git()` helper now strips it instead of inheriting it | DL-001 |
| 2026-08-04 | T004 | Noteworthy | The three duplicated issue-class -> E-code maps had ALREADY drifted: `address-path-escape` answered to E405 in one verb and E433 in another | One exported map; the specific code wins (E433). Behaviour change to a filled body, flagged for review and now asserted by value | execution.log.md § T004, test/acts/dd-issue-codes.test.ts |
| 2026-08-04 | T004 | Noteworthy | Wiring `autoRegenerateSibling` would have dragged `graph`/`links` across the render boundary P3/P4 were split along (arch-check 2 -> 4) | Pure module move into `acts/dd/shared.ts`; baseline held at 2 with the rule live | execution.log.md § T004 |
| 2026-08-04 | T006 | Magic wand | The renderer derives a dynamic-key map's interior column order from the data, though `valuesShape` now declares it | Left as a follow-on; passing the shape down would give declared column order for free | MW-001 |
