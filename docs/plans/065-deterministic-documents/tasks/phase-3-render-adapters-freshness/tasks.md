...
flowchart TD
    subgraph P3["Phase 3 — render, adapters & freshness"]
        T001["T001 golden corpus + fixtures"]:::pending --> T002["T002 pure renderer"]:::pending
        T006["T006 leaf rulings"]:::pending --> T002
...
| [ ] | T006 | Leaf rulings (recorded in execution.log.md, one-line rationale each): (a) E42x code names/allocation within the frozen block; (b) `dd build`/`--check` exit mapping; (c) golden-file update procedure (regen script vs hand-edit) | harness-cli | — | All three recorded before their consumers land | leaf decisions are the coder's, constrained by the freeze |
