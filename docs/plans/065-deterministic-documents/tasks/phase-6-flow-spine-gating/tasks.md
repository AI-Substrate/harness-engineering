...
flowchart TD
    subgraph P6["Phase 6 — flow-spine gating (terminal)"]
        ...
        T004 --> T006["T006 live dog-food, two lenses"]:::completed
        T005 --> T006
        T006 --> T007["T007 docs + full validation"]:::completed
    end
```
...
|------|------|------|-----------|------------|------------|
| 2026-08-04 | T001 | Noteworthy | dd's `MemoizingDocLoader` caches for exactly one command; a fixture sharing one deps object silently spans what production treats as separate invocations, so a mid-test edit reads back the cached pre-edit sha with no error | Fixture exposes `freshDeps()`, modelling the next command | `DL-001`; gate-fixtures/index.ts |
| 2026-08-04 | T003 | Noteworthy | `dd_link.reading` needed beyond the dossier's declared `{address, gate, basis_sha}` trio, because T005 requires the pure renderer to badge from stored state | Asked the PM before building; approved with the "display-only, never gate truth" rider, which is pinned in both directions | PM ruling 2026-08-04 |
| 2026-08-04 | T006 | Noteworthy | `orient` printed a rail saying "not yet evaluated" directly above a freshly-computed gate block — each half correct, the pair unreadable; invisible to tests | `orient` rails from a doc carrying the live reading; renderer stays pure | `WIN-001`; execution.log 6.6(b) |
| 2026-08-04 | T006 | Noteworthy | The refusal named bare item ids, so "do the work" and "unblock someone" looked identical | Each incomplete item now carries its state | `WIN-001` |
| 2026-08-04 | T006 | Deferred | The `--force` override event wrote a resolved ABSOLUTE path into the committed flow document — diff noise + username leak | Dropped the key (the repo-relative address already identifies the doc); pinned that no fixture root appears in event details. General rule worth adopting: nothing a mutation writes into a flow doc may be machine-specific | `DL-002` |
| 2026-08-04 | T006 | Noteworthy | ~4.7s of every spawned CLI invocation is telemetry capture (98% of a 4.8s call); nothing signposts it | `HARNESS_NO_TELEMETRY=1` in the child env: journey suite 82s → 1.9s | `MW-001` |
| 2026-08-04 | T007 | Noteworthy | `test/integration/docs.test.ts` flake (default 5000ms timeout vs a ~5.16s spawned command) — outside this phase's fence | Measured, observed, reported to the PM rather than fixed across the fence; PM took it as `68d62225` | `DL-003` |
