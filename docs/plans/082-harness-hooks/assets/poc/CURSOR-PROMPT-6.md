# Cursor prompt 6 — connectivity AND commit, in one run
...
1. Run this command and keep its **entire** output for your report:
   ...
   ```
   python3 tools/env-report.py DURING-COMMIT-RUN
   ```

2. Create `AI3.md` — three or four lines about what this repo is for.
3. Add `export const four = 4;` to the **end** of `seed.mjs`.
4. Stage **everything**: `git add -A` — this will include changes you did not make, and that
   is intended.
5. Commit with the message: `feat: AI3.md and four (controlled run)`
