...
| T012 (run …0d3a) | **HIGH** | **H5 idempotency consumes unpushed telemetry after crash-before-push** — `refTree(ref)===tree` returned `pushed:false` WITHOUT pushing; the local ref is set by `updateRef` BEFORE the push, so a crash in between leaves the local ref ahead of the remote, and the next sync then consumes the buffer without ever delivering | **FIXED** — the idempotent path now skips only the duplicate COMMIT and still re-pushes `ref:ref` (idempotent on the remote: no-op if current, ff if behind). Two regression tests (re-flush re-pushes; a failed re-push surfaces + doesn't consume). Real data-loss bug I introduced in T012. |
| T012 (run …0d3a) | MEDIUM | **H4 "never collide" overclaimed** — a 32-bit FNV digest is too narrow a budget for an absolute claim | **FIXED** — widened to a ~64-bit double-hash digest + softened wording (comment, test names, doc) to "negligibly unlikely"; added a digest-width test. |
| T016 (run …0d3a) | MEDIUM | **doc states unresolved guarantees as true** — "never lost" / "never collide" while the H5 hole + H4 overclaim stood | **FIXED** — with H5 fixed the "never lost" claim is true again; corrected the idempotent-re-push description + softened the entropy claim in `telemetry-otlp.md`. |

**Run 0d3a (phase drain review)**: companion reviewed T012+T014, T015, T005, T016/docs — **APPROVED T015 + T005**, **REQUEST_CHANGES** on the 3 findings above (the HIGH a real bug I introduced in T012's H5). All 3 fixed. The idle stand-down meant the queued T012/T014/T015/T005 pings were all reviewed in ONE drain after `control:stop`. Ended `degraded`/`validated:false` on the same `findings[].id` minih schema bug (3rd fresh occurrence); the issue draft + magicWand stand.
