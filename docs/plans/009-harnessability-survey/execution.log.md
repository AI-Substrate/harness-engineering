...
### T016 + T016b · G5 dogfood verb (.harness/extensions/validate-harnessability.ts)
**Files**: `.harness/extensions/validate-harnessability.ts` (NEW — this repo's first extension)
- T016 (scaffold + options + clone): scaffolded via `harness new validate-harnessability` (add-extension's `harness new` path). Declared verb (name/summary/description) + options `--repo <urls...>` (variadic = one-or-more URLs; the contract's `command.option` has no repeatable collector, so variadic is the supported override mechanism), `--keep`, `--model <model>`. Makes `/tmp/harnessability-selftest-<iso>/`, `git clone --depth=1` each default repo (chalk/JS, byteorder/Rust, pflag/Go). No `node:*` import; all I/O via `ctx.exec`.
- T016b (background-fire + capture + envelope): per clone fires `bash -c 'nohup minih run … --skill-source path:skills --skill harnessability-assessment > run.log 2>&1 & echo $!'` (detached, survives verb exit), captures PID from stdout, then **launch-time run-ID capture** via polling `minih last-run` for a runId newer than a pre-fire snapshot (≈3.6s cap). Returns `ctx.ok`/`ctx.degraded` with `data.runs[]` (repo/url/dest/cloned/fired/runId/runDir/logPath/pid) + a runnable `next_action` (poll commands + 4-point validation checklist + cleanup). Error codes: `E_MINIH_MISSING` (hard error), `E_TMP`, `E_NO_CLONES` (hard error), per-repo clone/fire/capture degrade. Never throws.
**Evidence**: `harness doctor` → `validate-harnessability` `status: loaded`; `harness help` → `installed:1, failed:0`, verb listed. (`.harness/extensions/` is outside biome's scope; validated by load.)

### T017 · G5 minih worker agent (validate-harnessability-assessment-skill)
**Files**: `agents/validate-harnessability-assessment-skill/{agent.json,prompt.md,input-schema.json,output-schema.json,instructions.md}` (NEW)
- Mirrors `agents/install-and-validate-test-extension/` shape. Param `targetRepo` (required) + `mode` (default static).
