...
**Kind**: flight-plan · **Now**: review-1 · **Next**: ship · **Intent**: Vendor SDD the-flow into this repo as the 'builder' skill (+ its deps validate-v2, constitution, extract-domain), add a thin the-flow redirect, bake skills into the npm build for GitHub-free local-path install, and make harness update re-apply skills by the last-recorded method (targets/scope) via a skills lock. Coexist with eng-harness-flow (future merge). · **Nodes**: 10 · **Events**: 17
