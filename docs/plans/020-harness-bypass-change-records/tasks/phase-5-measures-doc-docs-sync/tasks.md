...
flowchart TD
    ...
    subgraph Phase["Phase 5: Measures doc + docs sync"]
        ...
        T002["T002: record-and-record-types.md<br/>(2 types + provenance + win)"]:::done
        T003["T003: AGENTS_README.md (win kind)"]:::done
        T004["T004: manifest entry + gen:docs"]:::done
        T005["T005: VERIFY full CI gate"]:::done
        T001 --> T004
        T002 --> T004
        T003 --> T004
        T004 --> T005
...
|------|------|------|-----------|------------|------------|
| 2026-06-16 | T002 | decision | Live companion (MEDIUM, Domain Compliance) flagged that the provenance section described an unset `agent` as **"omitted"**, but `provenance.ts` always stamps all 7 keys and renders a missing `HARNESS_AGENT` as `agent: null` — so `agent` is a **nullable stamped** key, not optional. "Omitted" undercut the frozen-8-key-contract claim. | Verified against `provenance.ts` (`agent: string \| null`; `yamlScalar(null)='null'`; header maps over all `PROVENANCE_KEYS` unconditionally) + `record-service.ts:194` (`envOrNull(env.get('HARNESS_AGENT'))`). Corrected **both** `record-and-record-types.md` and `harness-value-measures.md`: the key is always present, only the value is null when unset. | `provenance.ts:28-49`; companion finding `01KV7XY0Q6TZZQNTCRT3GYYJ4G` |
