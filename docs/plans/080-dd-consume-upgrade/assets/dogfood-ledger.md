...
- **2026-08-09, trigger 1 (32 files outside the touch set), phase-3 deletion**: every
  ...
  32 IN the deletion commit — the drift is the deletion's own consequence, the diff is
  one identical line per file (banner-only, verified by regenerate-diff-restore), and
  the alternative converts the corpus one file at a time as unexplained churn inside
  ...
  prescription of the standalone CLI must be spelled `npx dd` (or
  `node_modules/.bin/dd`), never bare.
