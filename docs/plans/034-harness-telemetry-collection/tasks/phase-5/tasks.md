...
| 5.8 | Session-end flush (trailing tail) or document accepting tail loss | [x] | commit 10 | RESOLVED: session-end flush via existing `harness telemetry sync` (preamble captures the tail delta → act flushes); no new path (tiling property). §6 resolved + tail-capture test |
| 5.9 | Update guide `telemetry.md` (v2) + extend `harness-value-measures.md` (working-time/flow-stage) | [x] | commit 11 | telemetry.md "The event stream (v2.0)" + session-end-flush; value-measures §(e) v2.0 example + working-ratio/flow-stage/outcome surfaces; design docs reconciled `events`→`event_stream` (AC-10/AC-11) |
