...
- **AC-06**: Telemetry never appears in a feature branch's working tree or PR diff — capture writes only to gitignored `.harness/temp/telemetry/`; durable writes go to `refs/harness-telemetry` via plumbing without touching the index/working tree (asserted: `git status --porcelain` unchanged across a capture + flush).
- **AC-07**: The sync step writes a commit to `refs/harness-telemetry` authored by a **non-individual identity** (default `harness-telemetry <noreply@…>`) — **no `git config user.email` is stored at the storage layer by default** (see §T1, AC-13). It pushes only that single refspec, best-effort; no other ref/branch is modified; a failed push leaves the buffer intact for retry (AC-14).
...
- **AC-13** *(constitutional gate — added by validation)*: A test confirms the orphan-ref commit author is the non-individual identity (not the engineer's `git config user.email`) and that the serialized segment contains no per-individual identity field. Directly gates the §T1 failure consequence (per-individual surveillance).
...
| 4 | Durable sync: GitWritePort + orphan ref + docs | git / sync / docs/measures | Flush buffer → `refs/harness-telemetry` via plumbing; guide + measures contract | Phase 1 |
...
| 3.4 | Verify zero stdout/exit drift + **structural perf sensor** on representative core commands (golden) | cli-kernel | Output byte-identical with/without telemetry on `doctor`/`flow`/`record`; capture path performs **no full-transcript read** (cursor-incremental only — asserted via FakeFs call inspection, **not** a wall-clock threshold) | AC-01; perf **reshaped** from the flaky `<10ms` wall-clock by grill-agent-done; rollback strategy unchanged |
...
| 4.2 | Implement `GitWritePort` + `ExecGitWrite` (commit to `refs/harness-telemetry`, **non-individual author**, no index touch) | git | `git status --porcelain` unchanged across write; author ≠ engineer email | AC-06, AC-07, AC-13 |
...
| 4.4 | Implement sync-service + core `harness telemetry sync` verb + best-effort single-refspec push (ambient git auth) | sync | Pushes only `refs/harness-telemetry`; failed push doesn't error host; no other ref touched | AC-07, AC-14; verb named |
...
| AC perf | deterministic | **structural** no-full-read via FakeFs (3.4) | capture reads ≤ the windowed byte range | flaky wall-clock that passes on a fast CI run, hides an O(transcript) read | — |
