...
4. **copilot-vscode file evidence** — ~~same blind spot~~ **closed on this branch (phase 2, `cb95268b` + `2208e217`)** via the same specimen process: a real VS Code Copilot run (`demo/linestat`) exposed the store's `session_files` table (path + tool_name + first_seen_at, UNIQUE per file). Shipped: `files` written/edited lists (explicit write-tool allowlist — reads/`list_dir`/unknown/NULL excluded, so authorship means *wrote*; written-vs-edited from the chat-editing state's truncated initial-content hash, degrading to all-`edited` when absent) and a `tools` histogram (per-file-first-seen ceiling). **Deliberately NO `file` events**: the store persists no patch payloads, so deltas are unknowable — fabricated zeros were forbidden; the honest ceiling is paths-without-lines. Store-shape facts recorded for posterity: `initialFileContents` is an array of `[uri, hash]` pairs with the hash truncated to 7 chars; `session_files.file_path` is a raw absolute path while the editing state keys percent-encoded `file://` URIs (normalized for comparison only); `list_dir` rows are *directories* — the fact that makes any "touched ⇒ authored" rule wrong. Independent terra review: round 1 FIX_REQUIRED (high — an untimed turn at the window boundary open-ended the row windowing, double-counting authorship; reviewer supplied a reproducer), fixed with seek-forward timed boundaries + the reproducer as a 3-window regression test; round 2 APPROVE with mutation evidence.
