...
the design doc is `structural-proof-graph.md` beside it). Originally drafted
as 069; renumbered when `069-discipline-signal-capture` landed on main the
same day.
...
- "you could convert our current plan to a dd please. let's try to dogfood
  this as much as possible" — and then, sharper: the dd must carry the FULL
  builder plan spec ("this is the real deal"; if the schema is wrong, fix
  the schema now).
- "in future there will not be a -plan.md, just dd."
