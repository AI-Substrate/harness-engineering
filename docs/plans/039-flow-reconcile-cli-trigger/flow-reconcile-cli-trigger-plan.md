...
| [ ] | T014 | IMPL: the-flow real **harness-agnostic** `flight-plan.template.json` — **BARE SPINE ONLY**: exactly the 4 spine nodes `research → plan → phase-1 → ship`, **complete at create**, **strip EVERY harness node** (no backpressure/boot/observe/retro). The prototype's harness chores represent the *post-T020-conditional-apply* state, **not** the shipped template — port only the spine, not the seams (R2b) | the-flow template | `~/github/tools/.../flight-plan.template.json` | `create` + `render` of a no-harness repo shows the 4-node spine, zero harness nodes | AC-13; AC-09(1); R2 |
| [ ] | T015 | IMPL: the-flow **plan-complete expander** — emits ONE **purely additive** `apply` batch (stdin) splicing `phase-2..N` after `phase-1`; **gated (router installed AND provisioned)**: gate on → each new phase carries its own boot+observe+drain chores (deterministic ids `boot-N`/`observe-N`/`retro-N`); **gate off → bare phase nodes only** (R1 — no half-harnessed spine); **nothing relocates, no `mv`**; **byte-stable idempotent** (no-ops on a complete spine) | the-flow expander | `~/github/tools/.../` (expander routine) | Harnessed: expanding a 2-phase plan reproduces `the-flow.expanded`; no-harness: bare phase nodes only; re-run byte-stable no-op | AC-13; AC-12; AC-03; R1 |
...
| [ ] | T017 | IMPL: the-flow `harness-seams.md` — three-part creation + observe-gets-a-chore + drain/harvest + **AC-14 chore-execution discipline**, **verbatim-identical** to Phase 1's SKILL.md (T012); wrap the shared block in **BOTH** files with a parity marker `<!-- doctrine-parity:039 v1 — edit BOTH copies; keep byte-identical -->` (R4 — future-edit drift guard) | the-flow doctrine | `~/github/tools/.../harness-seams.md` | `diff` of the marked block vs SKILL.md is empty; marker present in both | AC-09; AC-14; R4; T012 |
