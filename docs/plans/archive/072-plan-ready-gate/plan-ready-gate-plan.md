...
| [x] | T017 | R4/F001: recognize the doctrine's three basis-less completed-attempt shapes | `flow` | `services/flow/chores-read.ts`, `test/services/dd/plan/ready.test.ts` | Detection receipt plus parsed JSON `noop` and `UNAVAILABLE` envelopes read `missing-basis`; other basis-less validation remains not satisfied | R4 ruling |
| [x] | T018 | R4/F002: give receipt-present invalid states a truthful reason | `dd/plan`, `flow` | `services/dd/plan/ready.ts`, `services/flow/chores-read.ts`, `test/services/dd/plan/ready.test.ts` | Invalid decision/validation comments read `invalid-receipt`, never `missing-receipt` | R4 ruling |
| [x] | T019 | R4/F003: keep base fallback behind later green evidence | `flow` | `services/flow/chores-read.ts`, `test/services/dd/plan/ready.test.ts` | Without the exact current id, later satisfied validation or decline outranks stale plain-node fallback | R4 ruling |
