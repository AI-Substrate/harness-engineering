...
|----|---------|---------|-----------|--------|--------|
| [FX001](./fixes/FX001-capture-bash-command-signature.md) | 2026-07-01 | Keep the already-computed non-harness command signatures in the event stream so `bash_command` keys by `rg`/`git` (argv granularity), not just tool name; **+ Facet B skill leading-digit capture** (`/the-flow 08`→`08`) | telemetry / _adapters | Complete | Phase-2 discovery (observe MW-001) + principal directive |
| [FX002](./fixes/FX002-honest-attribution-rules.md) | 2026-07-01 | Honest report-time attribution: per-dimension tokens = **`input`+`output` separate, non-cache** (cache session-only); commands = count+in/out **no time**; skills = skill-to-skill window; flow_stage = between-`/the-flow`-calls bracket; idle excluded everywhere. Capture untouched. | telemetry | **Complete** — cross-model reviewed (gpt-5.5, Dim-0 ×6) + live-sniff-verified; see [reviews/fx002-fx003-review.md](./reviews/fx002-fx003-review.md) | Live-data review (arch-check showed 22h / 36.2M tokens) |
| [FX003](./fixes/FX003-capture-tool-result-size.md) | 2026-07-01 | Capture per-tool-call **result size** (`ToolsEvent.result_tokens`, a count) so FX002's command `input` is **byte-weighted** — the "which call dumped 200k back?" signal. Capture-side additive (mirrors FX001 threading); FX002 falls back to even-split for old data. | telemetry / _adapters | **Complete** — bundled with FX002; 1 HIGH (segment-schema) found+fixed+re-reviewed; see [reviews/fx002-fx003-review.md](./reviews/fx002-fx003-review.md) | Live-data attribution design (per-single-call tokens) |
