...
7. **AC-07** — A **`--check` drift guard** following the same *drift contract* as `scripts/flow-fixtures.mjs` (regenerate → assert no divergence → non-zero on drift), that regenerates goldens and **fails when a committed golden diverges** from a re-derive; wired to a `just`/npm target and runnable in CI. Because the plan ships **no public CLI verb** (Non-Goal), the regen entrypoint **orchestrates the two golden Vitest suites** (`real-capture.e2e.test.ts` + `copilot-vscode-sqlite.int.test.ts`) — which already own every surface's raw→segment construction over `src/` under `REGEN_GOLDEN=1` — rather than re-importing the built `dist/` adapter + `serializeSegment` and duplicating those builders. That keeps a **single source of truth** for segment construction (the literal `dist/`-import wording is superseded; the orchestration choice avoids the very drift AC-07 exists to guard). The guard also enumerates every committed `fixtures/real/<surface>/<instance>/` and fails on any instance no suite covers (decision record: Phase 3 execution log).
...
| # | Task | Domain | Success Criteria | Notes |
|---|------|--------|-----------------|-------|
| 3.1 | `scripts/telemetry-fixtures.mjs` — regenerate goldens by **orchestrating the two golden Vitest suites** (which own the per-surface raw→segment construction over `src/`) under `REGEN_GOLDEN=1`, and enumerate every committed instance; `--check` runs them plainly so their deep-equal golden/invariant assertions fail non-zero on divergence (single source of truth — supersedes the literal `dist/`-import wording; see Phase 3 log). No CLI verb (Non-Goal). | _tooling | `--check` passes clean; mutating a golden makes it fail; an uncovered instance fails the guard | AC-07; Finding 05; validator Finding 1 |
