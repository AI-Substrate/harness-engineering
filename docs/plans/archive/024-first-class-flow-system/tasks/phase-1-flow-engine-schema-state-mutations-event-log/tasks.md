...
| 2026-06-18 | T013 | gotcha | **Companion HIGH**: `insert-node` rewired existing nodes' `next[]` + fired `node-updated` but never bumped their `modified_at`; new nodes (`add-node`/`insert-node`) had only `created_at`. The datetime contract (ws-002 §E5 "modified_at every mutation") leaked. | Fixed: `materialize()` stamps `modified_at = created_at` (trio always queryable on a new node); `insertNode` bumps `modified_at` on every rewired existing node (--after target, --before predecessors). Tests added (add-node + --after + multi-pred --before). | T013; companion HIGH; ws-002 §E5 |
| 2026-06-18 | T015 | gotcha | **Companion HIGH**: `runMutation` wrote without the post-mutation `validateFlowDoc` that `flow-mutations.ts`'s own comment promised — a bad `--status`/`--type`/`--next` could atomically persist a schema-invalid flow. | Fixed: `validateMutatedDoc` re-resolves the overlay by `doc.kind` + `validateFlowDoc` before `writeFlowAtomic`; issues → `E300`, nothing written. Tolerant skip if the overlay can't be re-resolved (out-of-repo `--schema` not re-passed). Act-level tests (`test/acts/flow.test.ts`) prove bad status/type → E300 + file byte-unchanged. | T015; companion HIGH; AC-07 |
| 2026-06-18 | T014 | decision | **Companion MEDIUM** (anti-reinvention): `FlowProvenance` + `nextEventId` duplicated `record/provenance.ts` `ProvenanceFields` + observe's private `nextId`. | Partially addressed: `FlowProvenance` now **reuses** `ProvenanceFields` (type-only import — zero runtime coupling, can't drift). `nextEventId` KEPT (observe's `nextId` is private; extracting a shared `nextId(prefix,entries)` touches observe-service + its tests = Phase-2 cleanup, not Phase-1 scope). Format is independently frozen (buffer-codec `ID_PATTERN` + the T012 test). | T014; companion MEDIUM; Finding 04 |
