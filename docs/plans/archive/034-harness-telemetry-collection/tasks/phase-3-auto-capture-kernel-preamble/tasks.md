...
**ACs**: AC-01 (composition-root: invoked once / help+version excluded / byte-identical output) · AC-09 (fail-safe: a throwing capture never changes the host exit code) · AC-06 guard (T005 asserts, at the **FakeFs path level**, that every capture write lands under the self-ignoring `.harness/temp/` tree now that capture fires on every command — this is a path-level fake proof, **not** a real `git status --porcelain` proof, which stays at Phase-1 task 1.7 / Phase-4 task 4.2)
...
flowchart TD
    ...
    subgraph Phase["Phase 3: Auto-capture kernel preamble"]
        ...
        T002["T002: TEST composition-root — invoked once / help+version excluded (AC-01)"]:::completed
        T003["T003: TEST fail-safety — throwing capture, exit byte-identical (AC-09)"]:::completed
        T004["T004: IMPL thin preamble in main() (delegates to capture-service)"]:::completed
        T005["T005: TEST zero stdout/exit drift + structural perf sensor (AC-01, M-K6)"]:::completed
...
| AC-06 PR-invisibility not re-asserted under 100% write frequency | LOW | T005: all writes under `.harness/temp/` (FakeFs path-level proof; real porcelain at Phase-1 1.7) |
...
STOP — ✅ Phase 3 COMPLETE (T001–T005). All code landed, every commit companion-reviewed.
