...
- **Manual "anything bad" review** (non-skippable): dry-run staged to `scratch/`; user prompt = innocuous harness-test prompt; assistant reply innocuous; **leak scan 0** for `/Users/`, `C:\`, emails, `alice`/`example-handle`/`Alice Example`, api-key shapes, on both `raw.events.jsonl` and `raw.process.log`. Scrub rebased all paths to `/home/dev[/repo]`.
...
- **Manual "anything bad" review**: leak scan 0 across both raw files (`/Users/`, `C:\`, emails, `alice`/`example-handle`/`Alice Example`, api-key shapes); user prompt + assistant reply innocuous; vendor system prompt redacted.
...
- **Live mechanism proven** (feeds T007): `harness capture-fixtures --surface copilot-vscode --names "example-handle,Alice Example,alice" --dry-run` against the **real** 4 KB store → staged `raw.rows.json` for session `7fb3a97f` (8 turns). **Manual review**: unscrubbed→scrubbed diff = the cwd path ONLY (`/Users/alice/...` → `/home/dev/repo`); scrubbed file leak-scan **0**; **no message columns/prose** — bodies fully stripped by the projection (text never reaches a written file). All 8 turns timestamped → `event_stream` will populate, `t_precision:'anchored'`.
...
- **Manual "anything bad" review** (non-skippable): leak scan **0** (`/Users/`, `C:\`, emails, `alice`/`example-handle`/`Alice Example`, key shapes); **no message text** (projection-stripped); only structural rows + the rebased `/home/dev/repo` cwd. `meta.json` attests `scrub_categories` incl. git-handles + person-names.
