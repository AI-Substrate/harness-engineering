...
> This repo practices exactly that. The composite gate is **`harness checks`** (tests, lint, typecheck, drift guards, architecture, skills, markdown, Windows-compat) — one command. `just install-hooks` runs it before every push; CI runs it on every PR so nothing merges red. You don't fix the code and move on — you fix the *check*.
