...
**Product operations are not skill stage aliases.** `harness builder new|adopt|guide|ready|contracts|settings|dispatch|ack|advance|compose|review|close|tidy` expose deterministic records and checks. Load [`references/team-lifecycle.md`](./references/team-lifecycle.md) when operating a team; `commands.ts` is the frozen CLI grammar. The PM advances only the existing flow through `harness builder advance`; no second team lifecycle. Pristine pre-work AckReceipt `ack-<unit_id>-<full-current-source-sha>` precedes release; after observing it the peer emits fresh `ack-<unit_id>-<full-current-source-sha>-release`, returns its path/digest and follows the already-granted scope without a second grant. Both use `harness builder ack <plan> --receipt <path>`; the phase selects `packet.nonce` versus retained `release.message_id`, and PM confirmation must precede import of queued delivery. Import is not composed-artifact proof; preservation precedes retirement.
