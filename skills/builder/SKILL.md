...
name: builder
description: |
  The single front door to the SDD pipeline (docs/plans/): research → product plan → implementation guide → tasks → implement → review → post-flight → ship. Use to plan, research, architect, clarify, workshop, write an ADR, implement, review or ship. Product intent lives in plan.dd.json; architecture, ownership, waves and proof live in assets/impl-guide.dd.json. Guided mode follows one canonical flow. Direct jumps: /builder <id> <verb> — 1a explore, 1b plan, 2c workshop, 3a adr, 4 guide, 5 tasks, 6 implement, 6a progress, 7 review, 7b post-flight, 8 ship, 8c reconcile. Printed commands carry id and verb.
---
...
**Guided** — `/builder` (no args, or `<slug>` / `<ord>-<slug>`; `/the-flow` is the compatibility entry):
...
| 1b | plan | `references/stages/20-plan.md` | intent, dossier?, workshops? → product intent in `plan.dd.json` + generated view; auto-runs `/validate-v2` | `"<intent>"` `[--simple]` `[--skip-clarify]` |
...
| 4 | guide | `references/stages/40-impl-guide.md` | product plan, source contracts, backpressure → separate `assets/impl-guide.dd.json`, architecture-enabled decomposition and independent review; not the future code baseline | `--plan "<path>"` |
| 5 | tasks | `references/stages/50-phase-tasks.md` | plan + reviewed guide → `assets/tasks/phase-N/tasks.dd.json` + context brief | `--phase "<Phase N: Title>" --plan "<path>"` |
| 6 | implement | `references/stages/60-implement.md` | plan, guide, tasks → committed units + verified composition and execution receipts (one phase) | `--plan "<path>"` `[--phase "<Phase N: Title>"]` `[--subtask "<ORD-slug>"]` |
...
| 7b | post-flight | `references/stages/75-post-flight.md` | phases + reviews → closeout evidence, whole-plan completion at exit, archive and external preservation; separate ownership-checked retirement | `--plan "<path>"` |
...
**Product operations are not skill stage aliases.** `harness builder new|adopt|guide|ready|contracts|settings|dispatch|self-check|advance|compose|review|close|tidy` expose deterministic records and checks. Load [`references/team-lifecycle.md`](./references/team-lifecycle.md) when operating a team; `commands.ts` is the CLI grammar. Brief each worker with owned paths, allowed reads, the job and observable done conditions, then the exact packet pointer/digest. Receiving the work packet starts that unit; optional `harness builder self-check <packet> --sha256 <digest>` compares packet bytes, checkout root and source SHA with warning-only guidance, without recording permission or waiting for a separate release. Import enforces current packet/dispatch/allocation and source integrity, not startup receipts. The PM advances only the existing flow through `harness builder advance`; import is not composed-artifact proof, and preservation precedes retirement. Historical receipts remain unchanged and are never replayed as current prerequisites.

Both skills use [`references/backpressure-recipe.md`](./references/backpressure-recipe.md): local `node_modules/.bin/ddocs`, canonical `assets/backpressure.dd.json`, actual RUN/EXTEND/BUILD/ABSENT selection, every AC/assertion `pressure`, observed AC `proven_by`. Certainty is `Partial|Confident|Proven`.
...
| `plan-3-v3-architect` | `4 guide` |
| `plan-3-v2-architect` | `4 guide` |
| typed `specify` | `1b plan` |
| typed `architect` or legacy id `3` | `4 guide` |
...
7. **Team receipts are facts, not a second lifecycle.** The PM owns canonical plan/guide/flow/progress/receipt writes. Workers stay within their packets. Requested cross-model review must actually run; unavailable support never becomes an unannounced solo success. Completed historical plans keep their read path without injected guide gates.
...
- **Builder capability floor for new team plans.** Also probe `harness builder --help`, `harness builder guide --help`, `harness builder advance --help` and `node_modules/.bin/ddocs --help`. Require the public grammar in `references/team-lifecycle.md`; missing support is a named prerequisite, never a prompt-only fallback. Completed historical plans are inspection/resume inputs, not targets for new dispatch gates.
