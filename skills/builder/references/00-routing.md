...
| `awaiting-backpressure` | `assets/backpressure.dd.json` (+ generated `.dd.md`; pre-dd plans: `backpressure-coverage.md`) | → **plan** (re-run, **informed by** the coverage — advisory, not auto-read) | post-plan refinement hanging off `awaiting-1b`; backpressure payoff (artifact produced via the router) | Certainty (Strong/Partial/Weak) + Phase 0? |
...
| `awaiting-backpressure` | **ABSENT** / **BUILD** sensors (the eyeball-gaps); a recommended **Phase 0: Establish Backpressure** |
...
A plan folder keeps exactly **four files at its root** — `<slug>-plan.md`, `the-flow.json`, `the-flow.md`, `original-ask.md`. **Every other stage artifact lives under `${PLAN_DIR}/assets/`**: `assets/research-dossier.md` · `assets/workshops/` · `assets/tasks/<phase>/` (Full-mode reviews ride inside it: `assets/tasks/<phase>/reviews/`) · `assets/execution.log.md` · `assets/reviews/` (Simple-mode reviews) · `assets/backpressure.dd.json` (+ generated `.dd.md`) · `assets/post-flight.md` · `assets/ship/`.
