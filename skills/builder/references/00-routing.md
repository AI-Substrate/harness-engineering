...
1. Derive the slug from the intent and select an explicit isolated workspace and actor.
2. Use `harness builder new <slug> --workspace <path> --actor <id>`; it reserves the ordinal and records allocation before mutation. Do not race a local folder scan or create a second workspace.
3. For an existing workspace use `harness builder adopt <plan> --actor <id> --owner external|pij`, never claim harness ownership. Read the returned plan/flow/allocation paths.
4. Preserve the verbatim ask in `original-ask.md` and canonical `nav.intent`; all mutations use the CLI.
5. Offer **explore** for unresolved source questions or **plan** for clear intent. New and adopted team plans use `team-lifecycle.md`; capability gaps stop the affected operation explicitly.

**Create + seed block** — define the path **once** (real runs use `docs/plans/<ord>-<slug>/`; a scratch/eval run redirects only `PLAN_DIR`/`FLOW_PATH`). **Route A (plan 040 / D1): the FULL 12-node seed is instantiated from the pre-authored template in ONE `create --template` call** — the seed is complete-at-create (the `research → plan → impl-guide → phase-1 → review-1 → post-flight → ship` spine **plus** the 5 harness chores and pre-authored per-node `instructions[]`, all baked into the template), so there is **no per-node `add-node`, no last-to-first build-order wart, and NO create-time chore apply** (`create` stamps the whole seed in ONE atomic source write; later structural edits use the batched `apply`, which resolves forward `--next` refs in one in-memory pass and commits it with that same single atomic source write — atomicity is that write, not the whole verb: the sibling `.md` refresh runs after it and can still refuse the operation, and its rollback is an ordinary rewrite — `flight-plan-ops.md` §6). **There is no router/provisioned gate at create** — D1 reversed the 039 split: the chores are mandatory for the agent and declinable only by the human (doctrine block, `harness-seams.md`); an un-harnessed repo simply carries them un-run (harmless — no router, no call), and even a weak model gets the whole spine + chores + instructions with **zero inference**:
...
TEMPLATE="<skill base>/references/flight-plan.template.json" # the FULL 12-node seed (research → plan → impl-guide → phase-1 → review-1 → post-flight → ship + 5 chores + per-node instructions[])

# root + the complete 12-node seed in ONE call — cursor sets nav.now; seven spine nodes
...
harness flow create flight-plan --slug <slug> --path "$FLOW_PATH" --schema "$SCHEMA" --template "$TEMPLATE" --plan-dir "$PLAN_DIR" --agent the-flow
...
# already the full 12-node starter with zero conditional logic and no router/provisioned gate. (Multiphase: the
...
The explicit source-template block is for direct flow creation/evaluation; `builder new` already creates the canonical flow, so never run both and allocate duplicate state. Team departures use `harness builder advance <plan> --now <node>`, which delegates to the existing canonical flow gates. Raw nav setters are metadata/historical mechanics, not a team-readiness bypass.
...
- **Found** → inspect its current evidence/basis; for team lifecycle departures use `harness builder advance <plan> --now <node>` only after prerequisites and chore receipts are satisfied. Existence alone is not approval. Print the next command from the Registry.
...
| `awaiting-1b` | product `plan.dd.json` | draft → fix **plan**; ready → **guide**; refinement → **workshop** | automatic `pre-coding` router seam, explicit human decline only; product readiness is not dispatch readiness | observable claims, scope and unresolved decisions |
| `awaiting-4` | `assets/impl-guide.dd.json` + independent decomposition approval | issues → **guide**; approved → **tasks** then **implement** (Simple may use its already scaffolded phase tasks) | guide/decomposition exit does not require future contract code; baseline sealing is an implementation substep | architecture, solo/coders rationale, ownership, waves, proof |
| `awaiting-2c` | newest `assets/workshops/*.md` | product decision → **plan**; implementation decision → **guide**; more questions → **workshop** | authoritative decisions invalidate affected downstream bindings | selected option |
| `awaiting-backpressure` | `assets/backpressure.dd.json` | product impact → **plan**; selected architecture/proof → **guide** | local ddocs source, AC/assertion pressure and observed proven_by; human-declinable survey | Partial/Confident/Proven and named gaps |
...
Read canonical `nav` first, then actual DD sections with `node_modules/.bin/ddocs get`. Artifact existence is discovery, not approval or execution proof. A guide file alone never authorizes dispatch.

| Predicate | Evidence |
|---|---|
| Product plan ready | `plan.dd.json#meta/status` is ready and product decisions are resolved |
| Implementation guide approved | valid `assets/impl-guide.dd.json` plus current independent decomposition receipt |
| Implementation ready | `harness builder ready <plan> --unit <id>` actually reports ready for current baseline/dependencies |
| Composed artifact proven | committed artifact SHA with `compose --verify` receipts and independent composition review |
| Closed | factual closeout progress, post-flight EXIT completion check and surviving preservation evidence |

Historical completed DD/Markdown plans retain their read path and bindings; do not inject `impl-guide`, reseal a baseline, rewrite history or reopen terminal nodes. Historical unified `*-plan.md` and split `*-spec.md` + `*-plan.md` remain recognized without conversion. A legacy spec with no plan remains incomplete. Explicit adoption into new team execution requires a reviewed guide before new dispatch, never a retroactive gate on historical completion.

Legacy command names translate through the Registry at read time (`specify` → plan, `architect` → guide). This command translation does not mutate old artifacts or their completed lifecycle.
...
| `awaiting-backpressure` | ABSENT / EXTEND / BUILD proof gaps, actual selected command, probe trail, remaining judgement and Partial/Confident/Proven certainty |
...
- **Create-seed (Route A, plan 040 / D1)**: [`flight-plan.template.json`](./flight-plan.template.json) — the **full 12-node** `--template` seed instantiated at `create` (`research → plan → impl-guide → phase-1 → review-1 → post-flight → ship` + the 5 harness chores + per-node `instructions[]`, all baked in; **no create-time gate**). The chore *shape* it bakes is owned by the `doctrine-parity:039` block ([`harness-seams.md`](./harness-seams.md), workshop 001 WS-2) — the template is one consumer of that single source of truth.
...
1. **Mutate for what just happened** (one call per change, `--path FLOW`) — exact flags: [`flight-plan-ops.md`](./flight-plan-ops.md) § 3:
   ...
   - advance → `harness builder advance <plan> --now <id>` for team plans; raw `nav set --next`/`--clear-next` only selects advisory next metadata. Historical flows retain their established CLI path. Never depart while due chores or terminal receipt comments are missing.
...
0. **Implementation guide — new team plans only.** Ensure `plan → impl-guide → phase-1`, with the source template's instructions and explicit preflight zone. This is one stage in the existing flow, not a team graph. Completed historical plans are inspection-only: do not backfill guide gates into them. Preserve whole-plan completion on post-flight EXIT; never arm it on an expanded review. The legal migration/expansion batch is `flight-plan-ops.md` §3c.

1. **Phases — all of them, each followed by review.** Read `plan.dd.json#phases` and the guide's implementation units; historical Markdown uses its old phase index. Add missing `(phase-N → review-N)` with explicit flight zone and bare-ordinal task gates, followed by post-flight and ship. Preserve the source instructions and terminal statuses. Whole-plan completion stays on post-flight EXIT, even when more phases are appended.
...
A new plan root holds `plan.dd.json`, generated `plan.dd.md`, canonical `the-flow.json`/`the-flow.md` and `original-ask.md`. Implementation architecture lives at `assets/impl-guide.dd.json`; survey at `assets/backpressure.dd.json`; observed proof at `assets/execution-log.dd.json`; tasks at bare-ordinal `assets/tasks/phase-N/tasks.dd.json`; team receipts under `assets/team/`. Research, workshops, reviews, post-flight and ship reports also live under assets. Historical Markdown root layouts remain read-only inputs; never duplicate new truth there.
