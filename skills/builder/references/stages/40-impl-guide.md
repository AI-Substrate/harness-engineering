...
6. Fill EVERY guide section: `meta`, `architecture`, `fan_out`, `capabilities`, `units`, `baseline`, `isolation`, `roles`, `checks`, `composition`, `review`, and relevant `risks`. Units declare `paths`, explicit `reads` owners, `interface`, `depends_on`, `wave`, `acceptance`, `proof` and role `pm|coder`. Write/read maps guide coders and PM, not source-access permission. The baseline unit precedes independent coders; the PM composition unit consumes their deliveries after them. Every dependency must precede its consumer; show overlapping maps and coordination needs rather than hiding them in notes.
...
9. Run `harness builder guide "${PLAN}" --check`. It checks structure, dependencies and links, NOT architectural judgement. Ownership-only findings (write overlap, read-map coverage, unmapped baseline files, owner declarations and map hints) are visible warnings, never readiness vetoes; real malformed data, ambiguous IDs and invalid executable checks remain errors. Preserve `file`/`owning_unit`/`stage` and cause/fix for declaration-only markers. Obtain independent decomposition review against the exact plan/guide digests; record through `harness builder review "${PLAN}" --receipt "<review DD path>"`. Requested but unavailable review stays unfulfilled, never relabelled as a solo success.
...
An out-of-map implementation edit needs no approval or justification. For an optional read-only comparison at any time, use `harness builder on-track <plan> [--unit <id>] [--from <ref>] [--to <ref>] [--untracked]`; no readiness, seal or receipt prerequisite, no writes, exit 0. Show its basis, warnings and issues rather than inferring permission from the exit code. Default includes tracked work; `--untracked` adds new paths and explicit `--to` is committed-only. See `../team-lifecycle.md` for basis selection and unavailable-comparison semantics.
