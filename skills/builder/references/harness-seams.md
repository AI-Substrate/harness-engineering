...
**Builder worker startup and maps.** Lead with owned paths, mapped reads, the job and observable done conditions, then the actual `builder/work-packet` pointer/digest. Receiving it starts the unit; optional `harness builder self-check <packet> --sha256 <digest>` is warning-only orientation and writes no state. Coder and PM write/read maps guide, never veto; out-of-map work needs no approval or justification. `harness builder on-track <plan> [--unit <id>] [--from <ref>] [--to <ref>] [--untracked]` uses the automatic comparison without readiness, seal, review or receipt prerequisites, writes nothing and exits 0: show `compared`, basis/full SHAs, warnings and actionable issues. Default includes tracked staged/unstaged work; `--untracked` explicitly adds new paths; explicit `--to` is committed-only. Import still enforces actual tree/branch/commit, immutable evidence, distinct peers and sealed ancestry, not startup receipts or release. Keep guide/delivery/import/verify `file`/`owning_unit` warnings visible for independent review; real structural, Git and product-check failures remain failures. User authorization for global/external operations and the harness hook model are unchanged. Historical receipts stay unchanged and are never replayed or reclassified.
