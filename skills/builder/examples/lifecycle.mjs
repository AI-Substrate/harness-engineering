...
export function lifecycleFixtures({ sourceSha, root, contractBytes, guideBytes, planBytes }) {
  ...
  const packet = { record_type: 'packet', id: `packet-${attempt}`, recorded_at, nonce, unit, plan, guide, baseline: { path: 'assets/team/baseline.dd.json', sha256: digest(JSON.stringify(baseline)) }, allocation: { path: `${dirname(root)}/allocation.dd.json`, sha256: digest(JSON.stringify(allocation)) }, source_sha: sourceSha, workspace: root, parent: 'example-pm', requested: role, forbidden: ['.the-flow-state.json', 'the-flow.json', 'the-flow.md', 'canonical plan/guide/tasks/receipts', 'other units', 'global settings', 'pushes'], instructions: [
