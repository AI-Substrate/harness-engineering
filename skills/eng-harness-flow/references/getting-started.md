...
| **Backpressure Check** · `--hook pre-coding` | `backpressure` verb | **You**, recommended, post-spec | After scoped work is defined, before you architect/build it. Surveys whether the work is *provable by deterministic sensors* (build/type/test/lint/smoke/boot/architecture/schema) vs inference; writes `backpressure.dd.json`; can offer ready-to-paste "done when `<sensor>` is green" lines to encode into your acceptance criteria (or flag where backpressure looks too thin); may recommend an optional "Phase 0: Establish Backpressure". Advisory — the sensors prove, never the LLM. Never blocks. |
...
| `backpressure` verb | Deterministic-sensor coverage survey for scoped work | `docs/plans/<ordinal>-<slug>/assets/backpressure.dd.json` |
...
| `pre-coding` | spec written, before you build | **fire** | `backpressure` verb | `backpressure.dd.json` |
