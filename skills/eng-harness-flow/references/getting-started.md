...
| **Backpressure Check** · `--hook pre-coding` | `backpressure` verb | **You**, recommended, post-spec | After scoped work is defined, before you architect/build it. Surveys whether the work is *provable by deterministic sensors* (build/type/test/lint/smoke/boot/architecture/schema) vs inference; writes `assets/backpressure.dd.json` (+ generated `.dd.md`) and links each acceptance criterion's `pressure` to the row that measures it — the "done when `<sensor>` is green" line lives in the row, nothing is pasted by hand (or flags where backpressure looks too thin); may recommend an optional "Phase 0: Establish Backpressure". Advisory — the sensors prove, never the LLM. Never blocks. |
...
| `backpressure` verb | Deterministic-sensor coverage survey for scoped work | `docs/plans/<ordinal>-<slug>/assets/backpressure.dd.json` (+ generated `.dd.md`; links the plan's criteria to its rows) |
...
| `pre-coding` | spec written, before you build | **fire** | `backpressure` verb | `assets/backpressure.dd.json` (+ `.dd.md`) |
