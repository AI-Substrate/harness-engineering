...
| improvement chosen | route the improvement → retro `[e]ncode` / `add-extension` / emit a fix-plan command / (harness-product friction in a consumer repo) an upstream issue on `AI-Substrate/harness-engineering` | the encoded harness change (or the filed upstream issue) |
...
- **Improve is where the loop compounds.** The loop only *compounds* when a retro leads to an encoded improvement. A single mid-plan session may encode nothing — but **`post-flight` (plan complete) must not end silently**: after harvest, name the highest-leverage candidate out loud and make the encode offer explicit (`[e]ncode` / `add-extension` / a fix-plan command / an upstream issue). The user may decline the offer; the agent may not skip making it.
