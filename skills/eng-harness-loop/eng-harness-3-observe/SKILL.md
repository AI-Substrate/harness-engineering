...
- `eng-harness-1-boot` reported a missing or "not declared" signal-readiness dimension that affected the current work
...
Use targets such as `project-sensor`, `runtime-inspectability`, `architecture-fitness`, `security`, `schema`, or `tooling` to make the missing signal visible to `eng-harness-4-retro --harvest` without changing the schema.
...
The buffer is append-only. Each call appends a single YAML block. The skill never reads existing entries to validate or modify them. `eng-harness-4-retro --drain` is the consumer that reads + drains.
...
- **No fix application**. Entries describe friction; encoding happens via `eng-harness-4-retro --drain [e]ncode` (stages a diff for review).
- **No prompting the user mid-session**. Bubble-up is exclusively at session end.
- **No buffer reading or curation**. That's `eng-harness-4-retro --drain` (drain) and `eng-harness-4-retro --harvest` (curate).
